...
3. 元组的特征
   ...
   - 可索引：可以使用索引访问元素。
     ...
     - **元组的不可变性**
       ...
       ```python
       t = (2, "mit", 3) 创建了一个包含多个元素的元组
       t[0] 表示访问元组 t 的第一个元素，结果为2（索引从0开始）
       (2, "mit", 3) + (5, 6) 表示两个元组的拼接，结果为一个新的元组 (2, "mit", 3, 5, 6)
       t[1:2] 表示对元组 t 进行切片操作，结果为 ("mit",)
       t[1:3] 表示对元组 t 进行切片操作，结果为 ("mit", 3)
       len(t) 表示计算元组 t 的长度，结果为3
       max((3, 5, 0)) 表示找出元组 (3, 5, 0) 中的最大值，结果为5（其他函数也可以在元组上工作，例如 sum）
       t[1] = 4 尝试修改元组 t 的第二个元素，这将导致错误，因为元组是不可变的，不能修改对象
       ```
